...
import { contributors, gitHubUser, publicRepos, readmeOrNull } from "./gh-http-get";
...
async function* generateChunk(repo: GitHubRepo, org: string): AsyncGenerator<string> {
    let content = '';

    const fallback = 'Nicht bekannt';
    content += `Projekt ${repo.name} und Beschreibung ${valueOrFallback(repo.description, fallback)} und Sprache und Technologie ${repo.language} \n`;
