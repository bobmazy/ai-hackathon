...
import {
  ...
  GitHubRepoContributorDTO,
  toGitHubContributor,
} from "./models/GitHubRepoContributor";
import {
  GithubRepoReadme,
  GitHubRepoReadmeDto,
  toGitHubRepoReadme,
} from "./models/GitHubRepoReadme";
import { GitHubUser, GitHubUserDto, toGitHubUser } from "./models/GitHubUser";
import { AxiosError } from "axios";

const client = await githubHttpClient();

export async function readmeOrNull(
  repo: GitHubRepo
  ...
  return await client
    ...
    .catch((error) => {
      console.error(
        `Error fetching README.md of ${repo.name}: ${
...
export async function gitHubUser(
  contributor: GitHubRepoContributor
): Promise<GitHubUser> {
  return client
